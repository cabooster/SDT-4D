...
def generate_mask_pair_4D(img):
    # img shape: (N, C, T, D, H, W)
    n, c, t, d, h, w = img.shape
    total_patches = n * t * d * h // 2 * w // 2
    mask1 = torch.zeros(size=(total_patches * 4,),
                        dtype=torch.bool,
                        device=img.device)
...
def generate_subimages_4D(img, mask):
    # img shape: (N, C, T, D, H, W)
    n, c, t, d, h, w = img.shape
    img = rearrange(img, 'b c t d h w -> (b t d) c h w')
    subimage = torch.zeros((n * t * d, c, h // 2, w // 2),
                           dtype=img.dtype,
                           layout=img.layout,
                           device=img.device)
    for i in range(c):
        ...
        subimage[:, i:i + 1, :, :] = img_per_channel[mask].reshape(
            n * t * d, h // 2, w // 2, c).permute(0, 3, 1, 2)

    subimage = rearrange(subimage, '(n t d) c h w -> n c t d h w', n=n, t=t, d=d)
    return subimage
...
if __name__ == "__main__":
    # img = torch.randn((2, 1, 16, 64, 64))
    # img = img.cuda()
    # mask1, mask2, mask3 = generate_mask_pair(img)
    #
    # noisy_sub1 = generate_subimages(img, mask1)
    # noisy_sub2 = generate_subimages(img, mask2)
    # noisy_sub3 = generate_subimages(img, mask3)
    # print(noisy_sub1.shape)
    # print(noisy_sub2.shape)
    # print(noisy_sub3.shape)

    img = torch.randn((2, 1, 5, 16, 64, 64))
    img = img.cuda()
    mask1, mask2, mask3 = generate_mask_pair_4D(img)

    noisy_sub1 = generate_subimages_4D(img, mask1)
    noisy_sub2 = generate_subimages_4D(img, mask2)
    noisy_sub3 = generate_subimages_4D(img, mask3)
    print(noisy_sub1.shape)
    print(noisy_sub2.shape)
    print(noisy_sub3.shape)
