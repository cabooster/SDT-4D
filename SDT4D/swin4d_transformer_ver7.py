...
class WindowAttention4D(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        self.window_size = window_size  # Wd, Wh, Ww, Wt
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5

        # Define the relative position bias table.
        # 2*Wd-1 * 2*Wh-1 * 2*Ww-1 * 2*Wt-1, num_heads
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(
                (2 * window_size[0] - 1) * (2 * window_size[1] - 1) *
                (2 * window_size[2] - 1) * (2 * window_size[3] - 1),
                num_heads
            )
        )

        # Build relative position indices for every pair in the window.
        coords_d = torch.arange(self.window_size[0])
        coords_h = torch.arange(self.window_size[1])
        coords_w = torch.arange(self.window_size[2])
        coords_t = torch.arange(self.window_size[3])

        # Create the 4D coordinate grid.
        coords = torch.stack(torch.meshgrid(coords_d, coords_h, coords_w, coords_t, indexing="ij"))
        coords_flatten = torch.flatten(coords, 1)  # 2, Wd*Wh*Ww*Wt

        # Calculate pairwise relative coordinates.
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wd*Wh*Ww*Wt, Wd*Wh*Ww*Wt
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wd*Wh*Ww*Wt, Wd*Wh*Ww*Wt, 4

        # Shift relative coordinates into non-negative index ranges.
        relative_coords[:, :, 0] += self.window_size[0] - 1
        relative_coords[:, :, 1] += self.window_size[1] - 1
        relative_coords[:, :, 2] += self.window_size[2] - 1
        relative_coords[:, :, 3] += self.window_size[3] - 1

        # Flatten the relative position indices.
        relative_coords[:, :, 0] *= (2 * self.window_size[1] - 1) * (2 * self.window_size[2] - 1) * (
                    2 * self.window_size[3] - 1)
        relative_coords[:, :, 1] *= (2 * self.window_size[2] - 1) * (2 * self.window_size[3] - 1)
        relative_coords[:, :, 2] *= (2 * self.window_size[3] - 1)
        relative_position_index = relative_coords.sum(-1)  # Wd*Wh*Ww*Wt, Wd*Wh*Ww*Wt

        self.register_buffer("relative_position_index", relative_position_index)

        # Attention projections and dropout layers.
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1] * self.window_size[2] * self.window_size[3],
            self.window_size[0] * self.window_size[1] * self.window_size[2] * self.window_size[3],
            -1
        )  # Wd*Wh*Ww*Wt,Wd*Wh*Ww*Wt,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wd*Wh*Ww*Wt, Wd*Wh*Ww*Wt
...
class SwinTransformer4D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.to_float:
            ...
            x = x.float()      #  1,1,96,96,96,20
        # x = self.patch_embed(x)# 1,24,16,16,16,20
